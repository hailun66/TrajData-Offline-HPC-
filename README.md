...
**排雷**：修复了多个官方源码中导致跑批中断的隐蔽 Bug（如 Waymo 测试集的空地图崩溃、Lyft 的 Numpy 版本冲突等）。

## 📦 包含的数据集与规模
通过本指南，你将获得一个包含以下数据的终极缓存库（Cache）：
1. Waymo Open Motion (全量 Train/Val)
2. nuPlan (Mini/全量)
3. nuScenes (全量 Trainval/Test/Mini)
4. Lyft Level 5 (全量 Train/Val)
5. Argoverse 2 (全量 Train/Val)
...
```Bash
	...
	conda 创建-n trajdata_env python=3.10 -y
...
1. 强制安装：pip install --no-deps nuplan-devkit-xxxx.whl

2. 手动补齐栅格与地图库：离线安装 rasterio, rtree。

3. 手动补齐 AWS 库：离线安装 aioboto3, wrapt, retry。

4. 手动补齐配置文件：从源码中拷贝丢失的 .yaml 文件：
